...
public class Day3 {
    ...
    public static void setWireOnBoard(int x, int y, int wireNumber, Map<Integer, Map<Integer, Integer>> wiresOnBoard) {
        if(!wiresOnBoard.containsKey(x)) {
            wiresOnBoard.put(x, new HashMap<Integer, Integer>());
        }

        if(!wiresOnBoard.get(x).containsKey(y)) {
            wiresOnBoard.get(x).put(y, wireNumber);
        }else{
            wiresOnBoard.get(x).put(y, 9999);
        }
    ...
    public static void main(String[] argv) {
        Map<Integer, Map<Integer, Integer>> wiresOnBoard = new HashMap<>();
        ...
        for(int wireIndex = 0; wireIndex < wires.length; wireIndex++) {
            ...
            for(String instruction : instructionOne) {
                ...
                for(int i = 0; i < movement; i++) {
                    ...
                    setWireOnBoard(x, y, wireIndex, wiresOnBoard);
                }
            }
        }

        List<Integer> distances = new ArrayList<>();
        for(Map.Entry<Integer, Map<Integer, Integer>> xMap : wiresOnBoard.entrySet()) {
