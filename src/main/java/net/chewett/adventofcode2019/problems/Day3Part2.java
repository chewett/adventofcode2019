...
public class Day3Part2 {
    ...
    public static int setWireOnBoard(int x, int y, int distanceTravelled, Map<Integer, Map<Integer, Integer>> wiresOnBoard) {
        if(!wiresOnBoard.containsKey(x)) {
            wiresOnBoard.put(x, new HashMap<Integer, Integer>());
        }

        int newDistanceTravelled = distanceTravelled;
        if(!wiresOnBoard.get(x).containsKey(y)) {
            wiresOnBoard.get(x).put(y, newDistanceTravelled);
            newDistanceTravelled += 1;
        }else{
            newDistanceTravelled = wiresOnBoard.get(x).get(y) + 1;
        }

        return newDistanceTravelled;
    }



    public static void main(String[] argv) {
        List<Map<Integer, Map<Integer, Integer>>> wiresOnBoard = new ArrayList<>();
        wiresOnBoard.add(new HashMap<>());
        wiresOnBoard.add(new HashMap<>());
...
//                "R50,L50,R1,U1,L1"
        ...
        for(int wireIndex = 0; wireIndex < wires.length; wireIndex++) {
            ...
            String[] instructionOne = wires[wireIndex].split(",");
            for(String instruction : instructionOne) {
                ...
                for(int i = 0; i < movement; i++) {
                    ...
                    distanceTravelled = setWireOnBoard(x, y, distanceTravelled, wiresOnBoard.get(wireIndex));
                }
            }
        }

        List<Integer> distances = new ArrayList<>();
        List<Integer> wireDistances = new ArrayList<>();
        for(Map.Entry<Integer, Map<Integer, Integer>> xMap : wiresOnBoard.get(0).entrySet()) {
            int xPos = xMap.getKey();
            Map<Integer, Integer> yMapping = xMap.getValue();
            for(Map.Entry<Integer, Integer> yMap : yMapping.entrySet()) {
                int yPos = yMap.getKey();
                int wireValue = yMap.getValue();

                Map<Integer, Map<Integer, Integer>> secondWires = wiresOnBoard.get(1);
                if(secondWires.containsKey(xPos) && secondWires.get(xPos).containsKey(yPos)) {
                    int manhatDistance = Math.abs(xPos) + Math.abs(yPos);
                    int combinedWireDistance = wireValue + secondWires.get(xPos).get(yPos);

                    System.out.println(xPos + "," + yPos + " Manhattan distance: " + manhatDistance);
                    System.out.println(xPos + "," + yPos + " Combined wire distance: " + combinedWireDistance);

                    distances.add(manhatDistance);
                    wireDistances.add(combinedWireDistance);
                }
            }
        }

        Collections.sort(distances);
        Collections.sort(wireDistances);

        System.out.println(distances);
        System.out.println(wireDistances);
